...
def check_binary_union():
    ...
    for fold in range(n_fold):
        ...
        for model in model_lst:
            ...
            filename = f"/mnt/ssd1/dataset/pneumo/sota_predictions/{model}/{fold}_{model}_valid/{fold}_{model}_valid_index.npz"
            tfz = np.load(filename)
            outputs, outputs_mirror, ids, gts = tfz["outputs"], tfz["outputs_mirror"], tfz["ids"], tfz["gts"]

            outputs = outputs/255.   # (N,H,W)
            outputs_mirror = outputs_mirror/255.
    ...
    for i in range(folds_models_mean.shape[0]):
        for j in range(folds_models_mean[i].shape[0]):
            for k in range(folds_models_mean[i][j].shape[0]):
                folds_models_mean_bin.append(binarize_sample((folds_models_mean[i][j][k], mask_thresh, min_size_thresh, dilation)))
