    ...
    def __getitem__(self, idx):
        ...
        if annotations[0] != "-1":
            for rle in annotations:
                ...
                if width != self.width:
                    mask_orig = imresize(
                        mask_orig, (self.width, self.height), interp="bilinear"
                    ).astype(float)
                mask += mask_orig
...
if __name__ == "__main__":
    ...
    for train_index, val_index in kf.split(df):
        ...
        dataset_train = SIIMDataset_Unet(
            df_train,
            "/mnt/ssd1/dataset/pneumo/train_png/",
            augmentations=1,
        )
        dataset_val = SIIMDataset_Unet(
            df_val,
            "/mnt/ssd1/dataset/pneumo/train_png/",
            augmentations=1,
        )
        ...
        for epoch in range(num_epochs):
            train_one_epoch(
                model, optimizer, train_loader, device, epoch, print_freq=200
            )
        ...
        lr_scheduler = torch.optim.lr_scheduler.CyclicLR(
            optimizer, base_lr=1e-6, max_lr=2e-4
        )
        ...
        for epoch in range(num_epochs):
            ...
            train_one_epoch(
                model, optimizer, train_loader, device, epoch + 35, print_freq=200
            )
