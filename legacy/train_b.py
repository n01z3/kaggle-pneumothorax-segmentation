...
if __name__ == "__main__":
    ...
    df_path = "/mnt/ssd1/dataset/pneumo/train-rle.csv"
    df = pd.read_csv(df_path)
    kf = KFold(n_splits=5, shuffle=True, random_state=seed)
    FOLD = args.fold

    for n, (train_index, val_index) in enumerate(kf.split(df)):
        df_train = df.iloc[train_index]
        df_val = df.iloc[val_index]
        if n == args.fold:
            break

    print("START training FOLD ", n)
    print(df_train.shape, df_val.shape)
    dataset_train = SIIMDataset_Unet(df_train, "/mnt/ssd1/dataset/pneumo/train_png/", augmentations=1)
    dataset_val = SIIMDataset_Unet(df_val, "/mnt/ssd1/dataset/pneumo/train_png/", augmentations=False)

    # ds_train = SIIMDataset_Unet("/mnt/ssd1/dataset/pneumo/train-rle.csv", "/mnt/ssd1/dataset/pneumo/train_png/", augmentations=1)
    ...
    train_loader = torch.utils.data.DataLoader(dataset_train, batch_size=2, shuffle=True, num_workers=4)
    ...
    for epoch in range(num_epochs):
        ...
        if valscore > bestscore:
            ...
            torch.save(model_ft, "outs/" + f'{seed}_' + model_name + "_" + str(IMG_SIZE) + "_bcedice.pt")
        lr_scheduler.step()
