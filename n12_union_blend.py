...
def main():
	...
	model_name = 'blend_union_sx50_sx101_se154'
	...
	for jj in range(len(subs)):
		print (subs[jj])
		df = pd.read_csv(f'submissions/{subs[jj]}')
...
#################################################################
	...
	for threshold in threshold_list:
		...
		for index, row in tqdm.tqdm(sample_df.iterrows(), total=len(sample_df)):
			...
			submission_df.to_csv(f'outs/union_submission_{model_name}_{threshold}.csv', index=False)
